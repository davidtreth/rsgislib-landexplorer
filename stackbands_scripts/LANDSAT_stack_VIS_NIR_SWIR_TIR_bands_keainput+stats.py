...
# and no other KEA files
#  
# Converts KEA files of each band to single stacked file
#  
# imports
import rsgislib
...
keaFileList = [f for f in dirFileList if ((f[-4:]=='.KEA')or(f[-4:]=='.kea'))]
# if this is a Landsat 8 image the third character of the filename is "8"
if keaFileList[0][2] == "8": 
...
# and the cirrus (band 9 , i.e. band 8 in stack)
    ...
    Bands_VIS_NIR_SWIR_FileList = [f for f in keaFileList if (f[-5] in bands)and(f[-6]=='B')]
    Bands_TIR_FileList = [f for f in keaFileList if (f[-6]=='1')and(f[-7]=='B')] #bands 10 and 11
...
if keaFileList[0][2] == "7" or keaFileList[0][1] == "7" or keaFileList[0][2] == "5" or keaFileList[0][1] == "5": 
    # select bands 1 to 5 and 7, excluding thermal IR band 6
    bands = '123457'
    Bands_VIS_NIR_SWIR_FileList = [f for f in keaFileList if (f[-5] in '123457')and(f[-6]=='B')or((f[-6] in '123457')and(f[-7]=='B'))]
